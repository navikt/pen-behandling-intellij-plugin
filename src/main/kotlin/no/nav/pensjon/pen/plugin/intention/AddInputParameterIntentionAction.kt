...
class AddInputParameterIntentionAction : PsiElementBaseIntentionAction() {

    override fun getFamilyName() = "PEN Behandling"

    override fun getText() = "Legg til input-parameter"
    ...
    override fun invoke(project: Project, editor: Editor?, element: PsiElement) {
        ...
        val dialog = AddParameterDialog(project, "Legg til input-parameter")
        if (!dialog.showAndGet()) return

        val paramName = dialog.paramName
        val paramType = dialog.paramType

        WriteCommandAction.runWriteCommandAction(project, "Legg til input-parameter", null, {
            val newText = ParameterCodeModifier.addInputParameter(text, paramName, paramType, isBehandling)
            doc.setText(newText)
